...
const MODEL_COST_PATTERNS: Array<{ match: (id: string) => boolean; cost: ModelCost }> = [
  ...
  {
    match: (id) => /claude.*sonnet.*1m|claude.*1m.*sonnet/i.test(id),
    cost: MODEL_COST_TABLE["claude-4-sonnet-1m"]!,
  },
  ...
  { match: (id) => /gpt-5\.1.*codex/i.test(id), cost: MODEL_COST_TABLE["gpt-5.1-codex"]! },
  ...
  { match: (id) => /grok.*4[.-]3/i.test(id), cost: MODEL_COST_TABLE["grok-4.3"]! },
...
export interface ProcessedModel extends CursorModel {
  ...
  rawRoutingByEffort?: CursorModelRoutingByEffort;
}
...
): Map<string, CursorModelRoutingByEffort> {
  const lookup = new Map<string, CursorModelRoutingByEffort>();
  for (const model of models) {
...
export function applyRawCursorModelId(
  ...
  applyCursorModelRouting(payload, resolveCursorModelRouting(payload, rawRoutingByEffortByModelId));
}
